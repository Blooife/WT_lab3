        ...
        <frm:form method="put" modelAttribute="cartItemsQuantities" action="${pageContext.servletContext.contextPath}/cart">
          <table class="table table-hover table-bordered text-center">
            ...
            <c:forEach var="i" begin="0" end="${cart.items.size() - 1}">
              <tr id="row${cart.items[i].book.id}">
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].book.id}"/>">${cart.items[i].book.brand}</a>
                </td>
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].book.id}"/>">${cart.items[i].book.model}</a>
                </td>
                <td class="align-middle">
                  <ul>
                    <c:forEach var="color" items="${cart.items[i].book.colors}">
                      <li>${color.code}</li>
                    </c:forEach>
                  </ul>
                </td>
                <td class="align-middle">${cart.items[i].book.displaySizeInches}"</td>
                <td class="align-middle">$ ${cart.items[i].book.price}</td>
                <td class="align-middle">
                ...
                <td class="align-middle">
                  <button form="deleteCartItem" class="btn btn-outline-dark border-dark"
                          formaction="<c:url value="/cart?phoneId=${cart.items[i].book.id}"/>">Delete</button>
                </td>
              </tr>
            </c:forEach>
          </table>
