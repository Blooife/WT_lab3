...
create table phone2color (
  phoneId BIGINT,
  colorId BIGINT,
  CONSTRAINT FK_phone2color_phoneId FOREIGN KEY (phoneId) REFERENCES books (id) ON DELETE CASCADE ON UPDATE CASCADE,
...
create table stocks (
  ...
  UNIQUE (phoneId),
  CONSTRAINT FK_stocks_phoneId FOREIGN KEY (phoneId) REFERENCES books (id) ON DELETE CASCADE ON UPDATE CASCADE
);